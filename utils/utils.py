...
# img_H = CONFIGS["MODEL"]["H"]
# img_W = CONFIGS["MODEL"]["W"]


def Focal_Loss(inputs, target,alpha=0.25, gamma=2):
    logpt = -nn.functional.binary_cross_entropy_with_logits(inputs, target, reduction='none')
    pt = torch.exp(logpt)
...
#     H, W = size
#     theta = line.angle()
#     alpha = theta + np.pi/16
#     if theta == -np.pi/16:
#         r = line.coord[1] - W/16
#     else:
#         k = np.tan(theta)
#         y1 = line.coord[0] - H/16
#         x1 = line.coord[1] - W/16
#         r = (y1 - k*x1) / np.sqrt(1 + k**2)
#     return alpha, r
...
def reverse_mapping(point_list, numAngle, numRho, size):
...
def visulize_mapping(b_points, size, filename,path):
    img = cv2.imread(os.path.join(path, filename))#change the path when using other dataset.
...
def CE_Loss(inputs, target, num_classes=21):
    n, c, h, w = inputs.size()
    nt, hc, ht, wt = target.size()
    if h != ht and w != wt:
        inputs = F.interpolate(inputs, size=(ht, wt), mode="bilinear", align_corners=True)

    temp_inputs = inputs.transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
    temp_target = target.view(-1).to(torch.long)
